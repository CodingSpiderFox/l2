...
wget https://images.wallpaperscraft.com/image/glare_rainbow_circles_background_20329_1920x1080.jpg
-O backgroundCustom.jpg
sudo cp backgroundCustom.jpg /usr/share/backgrounds/
echo "default-wallpaper=/usr/share/backgrounds/backgroundCustom.jpg" |
sudo tee /etc/lightdm/pantheon-greeter.conf
rm backgroundCustom.jpg
gsettings set org.gnome.desktop.background picture-uri
"file:///usr/share/backgrounds/backgroundCustom.jpg"
...
# fix Alt+Tab+Shift issue (otherwise switching through desktop windows
is delayed)
...
night-light-temperature 'uint32 3720'
...
sudo xdg-mime default io.elementary.files.desktop inode/directory
application/x-gnome-saved-search
sudo xdg-mime default io.elementary.files.desktop  $(grep MimeType
/usr/share/applications/io.elementary.files.desktop | sed
's/MimeType=//' | sed 's/;/ /g')

# Revert by executing:
# sudo xdg-mime default org.gnome.Nautilus.desktop inode/directory
application/x-gnome-saved-search
# sudo xdg-mime default org.gnome.Nautilus.desktop  $(grep MimeType
/usr/share/applications/org.gnome.Nautilus.desktop | sed
's/MimeType=//' | sed 's/;/ /g')

sudo apt purge -y epiphany-browser epiphany-browser-data pantheon-mail
audience io.elementary.code noise

gsettings set io.elementary.desktop.wingpanel.power show-percentage true
gsettings set org.gnome.settings-daemon.plugins.color night-light-enabled true
#gsettings set org.pantheon.desktop.gala.behavior hotcorner-topleft
'open-launcher'
#gsettings set org.pantheon.desktop.gala.behavior hotcorner-topright
'window-overview-all'
#gsettings set org.pantheon.desktop.gala.behavior
hotcorner-bottomright 'show-workspace-view'
#gsettings set org.pantheon.desktop.gala.behavior hotcorner-bottomleft
'show-workspace-view'
gsettings set org.pantheon.desktop.gala.behavior overlay-action
'wingpanel --toggle-indicator=app-launcher'
...
#gsettings set io.elementary.code.settings plugins-enabled
"['outline', 'preserve-indent', 'strip-trailing-save',
'brackets-completion', 'detect-indent', 'highlight-word-selection',
'editorconfig']"
...
dconf write /org/pantheon/desktop/gala/notifications/applications/gala-other/sounds
false
...
python3 utilityscripts/create-custom-shortcut.py 'show desktop'
'wmctrl -k on' '<Super>d'
python3 utilityscripts/create-custom-shortcut.py 'show desktop'
'wmctrl -k off' '<Shift><Super>d'
python3 utilityscripts/create-custom-shortcut.py 'open system
settings' 'io.elementary.switchboard' '<Super>i'
python3 utilityscripts/create-custom-shortcut.py 'screenshot of
selection' 'shutter --select' '<Shift><Super>s'
...
https://flathub.org/repo/appstream/org.gnome.Builder.flatpakref

#Remove unneeded languagepacks
sudo apt --purge autoremove wbulgarian wbrazilian wfrench witalian
wspanish wportuguese wdanish wdutch wpolish wukrainian wnorwegian
wcatalan wswedish wamerican wbritish wswiss

sudo cp etc/default/grub /etc/default/grub
sudo cp etc/rc.local /etc/

sudo cp etc/xdg/autostart/indicator-application.desktop /etc/xdg/autostart/
